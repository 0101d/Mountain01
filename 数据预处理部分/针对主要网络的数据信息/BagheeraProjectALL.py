...
csv_file = open('BagheeraPageAll.csv','w',encoding="gbk",newline='')
...
for i in range(0,4):
    ...
    icount = i*24
    url = "https://www.deviantart.com/popular-all-time/?section=&global=1&q=bagheera&offset="+str(icount)
    response = requests.get(url)
    #print(response.content.decode('utf-8'))

    #将网页源代码保存到本地MyBambi.html文件中
    file_obj = open('MyBagheeraAll.html','w')
    file_obj.write(response.content.decode('utf-8'))
    file_obj.close()

    #从bs4引入数据提取工具BeautifulSoup
    from bs4 import BeautifulSoup
    file_obj = open('MyBagheeraAll.html','r')#以读方式打开MyBambi文件
