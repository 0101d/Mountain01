...
import csv
#本次抽样百分之一，样品数量823份，只选取2页
#本次直接爬取了从0到144页面的所有相应信息，将信息保存在csv格式文件之中

#以下是csv转换区，提前做好csv表格准备
csv_file = open('BenjaminAll.csv','w',encoding="gbk",newline='')
...
for i in range(0,1):
    ...
    icount = i*24
    url = "https://www.deviantart.com/popular-all-time/?section=&global=1&q=winne+the+pooh&offset=0"+str(icount)
    response = requests.get(url)
    #print(response.content.decode('utf-8'))

    #将网页源代码保存到本地MyBambi.html文件中
    file_obj = open('MyBenjaminAll.html','w')
    file_obj.write(response.content.decode('utf-8'))
    file_obj.close()

    #从bs4引入数据提取工具BeautifulSoup
    from bs4 import BeautifulSoup
    file_obj = open('MyBenjaminAll.html','r')#以读方式打开MyBambi文件
   ...
   # writer = csv.writer(csv_file)
    ...
    for each_bambi in all_bambis.find_all('span',class_="info"):
            ...
            #print("'a'标签个数=", len(all_a_tag))
            #print("热度所在标签",len(all_hot_tag))
            
            if(len(all_a_tag)==2 and len(all_hot_tag)==1):
                bambi_artist = all_a_tag[1].text
                bambi_hot = all_hot_tag[0].text
            #print(len(all_a_tag))#检测两个标签的数量是否符合平常数值
            #print(len(all_hot_tag))
