...
import csv
#本次抽样百分之一，672份样品，28个页面
#本次直接爬取了从0到4页面的所有相应信息，将信息保存在csv格式文件之中
...
for i in range(0,30):
    ...
    icount = i*24
    url = "https://www.deviantart.com/popular-all-time/?section=&global=1&q=bambi&offset="+str(icount)
    response = requests.get(url)
    #print(response.content.decode('utf-8'))

    #将网页源代码保存到本地MyBambi.html文件中
    file_obj = open('MyBambiPageAll.html','w')
    file_obj.write(response.content.decode('utf-8'))
    file_obj.close()

    #从bs4引入数据提取工具BeautifulSoup
    from bs4 import BeautifulSoup
    file_obj = open('MyBambiPageAll.html','r')#以读方式打开MyBambi文件
